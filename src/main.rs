...
#[behaviour(out_event = "Events", poll_method="poll")]
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut gossipsub = Gossipsub::new(MessageAuthenticity::Signed(local_key), gossipsub_config).expect("Able to Gossip");
    ...
    loop {
        tokio::select! {
            line = stdin.select_next_some().fuse() => {
